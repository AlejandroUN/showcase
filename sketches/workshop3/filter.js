...
function preload() {
  video_src = createVideo(['/docs/shortcodes/resources/videos/nature.mp4']);
  video_src.hide(); // by default video shows up in separate dom
  maskShader = readShader('/sketches/workshop3/filter.frag', { varyings: Tree.texcoords2 });
...
function draw() {
  ...
  if (mask.checked()) {
	...
	//Move right
	 //maskShader.setUniform('mask', [0, 0, 0, 1, 0, 0, 0, 0, 0]);
	 //Move left
	 //maskShader.setUniform('mask', [0, 0, 0, 0, 0, 1, 0, 0, 0]);
  }
  else {
